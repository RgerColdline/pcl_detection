...
#include "config.hpp"
#include "obstacle_pipeline_config.hpp"  // 添加新配置支持
...
// 提取多个垂直墙面（支持使用或不使用法向量，使用 WallConfig）
...
std::vector<Object::Ptr> extractWalls(typename pcl::PointCloud<PointT>::Ptr cloud,
    ...
    int wall_num                                    = 1;
    int max_walls                                   = 4;  // WallConfig 没有 max_walls 字段
           ...
           wall_num <= max_walls)
    {
        ...
        for (const auto &wall : walls) {
            ...
            if (angle < config.angle_threshold || angle > (180.0 - config.angle_threshold)) {
                ...
                // 增大平面距离阈值到 0.5m，避免同一面墙被重复检测
                if (std::abs(d1 - d2) < 0.5) {
                    is_duplicate = true;
                    break;
                }
            }
...
// ============================================================================
template <typename PointT> class WallExtractor : public IObjectExtractor<PointT>
{
    WallConfig config_;
  ...
  public:
    ...
    std::vector<Object::Ptr> extract(PointCloudPtrT cloud, NormalCloudPtrT normals,
...
// ============================================================================
// extractWalls 重载版本（使用 WallExtractionConfig，新 Pipeline 专用）
// ============================================================================
template <typename PointT>
std::vector<Object::Ptr> extractWalls(typename pcl::PointCloud<PointT>::Ptr cloud,
                                      typename pcl::PointCloud<pcl::Normal>::Ptr /*normals*/,  // 不使用
                                      const WallExtractionConfig &config,
                                      const std::set<int> &excluded_indices = {}) {
    ...
    int wall_num = 1;
    int max_walls = config.max_walls > 0 ? config.max_walls : 4;
           ...
           wall_num <= max_walls)
    {
        ...
        if (z_component > config.normal_z_max) {
            // 法向量太接近 Z 轴，这是水平面（地面/天花板），不是墙面，跳过
            std::set<int> inlier_set(inliers_local->indices.begin(), inliers_local->indices.end());
            typename pcl::PointIndices::Ptr new_indices(new pcl::PointIndices);
            for (int idx : current_indices->indices) {
